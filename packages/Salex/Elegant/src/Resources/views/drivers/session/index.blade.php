...
@section('content-wrapper')
    <div class="auth-content">
        <div class="sign-up-text">
            {{ __('shop::app.customer.login-text.no_account') }} - <a href="{{ route('driver.register.index') }}">{{ __('shop::app.customer.login-text.title') }}</a>
        </div>

        {!! view_render_event('bagisto.shop.drivers.login.before') !!}

        <form method="POST" action="{{ route('driver.session.create') }}" @submit.prevent="onSubmit">

            {{ csrf_field() }}

            <div class="login-form">
                <div class="login-text">{{ __('marketplace::app.drivers.login-title') }}</div>

                {!! view_render_event('bagisto.shop.drivers.login_form_controls.before') !!}
