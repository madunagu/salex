@php
$reviewHelper = app('Salex\Marketplace\Helpers\Review');

$total = $reviewHelper->getTotalReviews($store);
@endphp

{!! view_render_event('bagisto.shop.store.review.before', ['store' => $store]) !!}

@if ($total)
@php
$reviews = $reviewHelper->getReviews($store)->paginate(10);

$avgRatings = $reviewHelper->getAverageRating($store);

$avgStarRating = round($avgRatings);

$percentageRatings = $reviewHelper->getPercentageRating($store);
@endphp
@if (! empty($accordian))
...
{!! view_render_event('bagisto.shop.store.review.after', ['store' => $store]) !!}