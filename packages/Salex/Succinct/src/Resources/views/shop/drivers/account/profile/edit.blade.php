...
<form method="POST" @submit.prevent="onSubmit" action="{{ route('driver.profile.store') }}" enctype="multipart/form-data">
    ...
    <div class="account-table-content">
        ...
        <div :class="`row ${errors.has('first_name') ? 'has-error' : ''}`">
            ...
            <div class="col-12">
                <input value="{{ $driver->first_name }}" name="first_name" type="text" v-validate="'required'" data-vv-as="&quot;{{ __('shop::app.customer.account.profile.fname') }}&quot;" />
        ...
        <div :class="`row ${errors.has('last_name') ? 'has-error' : ''}`">
            ...
            <div class="col-12">
                <input value="{{ $driver->last_name }}" name="last_name" type="text" v-validate="'required'" data-vv-as="&quot;{{ __('shop::app.customer.account.profile.lname') }}&quot;" />
        ...
        <div :class="`row ${errors.has('gender') ? 'has-error' : ''}`">
            ...
            <div class="col-12">
                <select name="gender" v-validate="'required'" class="control styled-select" data-vv-as="&quot;{{ __('shop::app.customer.account.profile.gender') }}&quot;">
                    ...
                    <option value="" @if ($driver->gender == "")
                        selected="selected"
                        @endif>
                        {{ __('admin::app.customers.customers.select-gender') }}
                    </option>

                    <option value="Other" @if ($driver->gender == "Other")
                        selected="selected"
                        @endif>
                        {{ __('velocity::app.shop.gender.other') }}
                    </option>

                    <option value="Male" @if ($driver->gender == "Male")
                        selected="selected"
                        @endif>
                        {{ __('velocity::app.shop.gender.male') }}
                    </option>

                    <option value="Female" @if ($driver->gender == "Female")
                        selected="selected"
                        @endif>
        ...
        <div :class="`row ${errors.has('date_of_birth') ? 'has-error' : ''}`">
            ...
            <div class="col-12">
                <date id="date-of-birth">
                    <input type="date" name="date_of_birth" placeholder="yyyy/mm/dd" value="{{ old('date_of_birth') ?? $driver->date_of_birth }}" v-validate="" data-vv-as="&quot;{{ __('shop::app.customer.account.profile.dob') }}&quot;" />
                </date>
        ...
        <div class="row">
            ...
            <div class="col-12">
                <input value="{{ $driver->email }}" name="email" type="text" v-validate="'required'" />
        ...
        <div class="row">
            ...
            <div class="col-12">
                <input value="{{ old('phone') ?? $driver->phone }}" name="phone" type="text" />
        ...
        <div class="row image-container {!! $errors->has('image.*') ? 'has-error' : '' !!}">
            ...
            <div class="col-12">
                <image-wrapper :button-label="'{{ __('admin::app.catalog.products.add-image-btn-title') }}'" input-name="image" :multiple="false" :images='"{{ $driver->image_url }}"'></image-wrapper>
