...
@section('page_title')
    {{ __('marketplace::app.merchants.login-title') }}
@endsection

@section('content-wrapper')
    <div class="auth-content">
        <div class="sign-up-text">
            {{ __('shop::app.customer.login-text.no_account') }} - <a href="{{ route('merchant.register.index') }}">{{ __('shop::app.customer.login-text.title') }}</a>
        </div>


        <form method="POST" action="{{ route('merchant.session.create') }}" @submit.prevent="onSubmit">

            {{ csrf_field() }}

            <div class="login-form">
                <div class="login-text">{{ __('marketplace::app.merchants.login-title') }}</div>
