...
namespace Salex\Bitpay\Listeners;
...
class Transaction
{
    ...
    public function saveTransaction($invoice)
    {
        ...
        if ($invoice->order->payment->method == 'bitpay') {
            if (isset($data['orderData']) && isset($data['orderData']['orderID'])) {
                $bitpayOrderId = $data['orderData']['orderID'];
                $transactionDetails = $this->binance->getOrder($bitpayOrderId);
